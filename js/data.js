...
АДАПТИВ И ТОЧНАЯ ПОСАДКА ХОЛСТА (критично для узкого окна превью AI Studio — БЕЗ обрезаний по краям и БЕЗ горизонтальной прокрутки!):
• CSS строго так: «html,body{margin:0;padding:0;width:100%;height:100%;overflow:hidden;background:#0e0f1a} *{box-sizing:border-box} canvas{display:block;width:100%;height:100%}». Никаких элементов шире 100% и никакого горизонтального скролла.
• Внутреннее разрешение холста = ИЗМЕРЕННЫЙ размер элемента, а НЕ хардкод (никаких 1280×800!): «const dpr=window.devicePixelRatio||1; const r=canvas.getBoundingClientRect(); canvas.width=r.width*dpr; canvas.height=r.height*dpr; ctx.setTransform(dpr,0,0,dpr,0,0);». Логические W=r.width, H=r.height.
• На событие resize И через ResizeObserver пересчитывай размер холста и ВСЮ раскладку. Игра точно вписывается в контейнер любой ширины, ничего не уезжает вправо/вниз.
• ВСЕ позиции и размеры — в долях от текущих W,H (не в пикселях). При узком/портретном окне (W<H или малая ширина) — РЕФЛОУ: панели вниз, меньше колонок, мельче карточки и шрифты, при нужде внутренняя прокрутка списков.
• ЗАГОЛОВКИ И ЛЮБОЙ ТЕКСТ обязаны влезать по ширине: подбирай кегль через ctx.measureText() и уменьшай его (или переноси на 2 строки), чтобы текст помещался в свою зону с отступами. Большой заголовок («STYLE STUDIO» и т.п.) НЕ должен вылезать за края — сначала измерь, потом рисуй.
• Базовый кегль ≈ min(W,H)*0.03; крупный заголовок — выбери максимальный размер, при котором measureText(текст).width ≤ 0.9*W.
• Мысленно проверь при УЗКОМ окне (~360 px шириной): нет горизонтальной прокрутки, заголовок и все панели целиком в кадре, ничего не обрезано и не сплющено.
