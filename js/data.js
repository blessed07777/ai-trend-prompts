...
const TECH = `TECH SPEC:
• One single self-contained .html file, no external libraries, no CDN.
• HTML5 Canvas, ~60 FPS game loop via requestAnimationFrame with delta-time.
• Works on desktop (keyboard + mouse) AND mobile (touch). Canvas scales responsively.
• Crisp pixel/vector art drawn in code, smooth easing, particle effects, screen shake on impact.
• Pleasant pastel UI, rounded HUD, soft shadows, large readable score.
• Sound via WebAudio (short blips, no asset files). A mute button in the corner.
• Start screen → gameplay → game-over screen with final score, "best" saved to localStorage, and a "Play again" button.
• No bugs, no console errors, fully playable from the first click.`;

const DEVCARD = `DEVELOPER CARD (render as a stylized rounded card on start & end screens):
   ┌────────────────────────────┐
   │  🍌 AI Trend Studio         │
   │  Developer: {name}          │
   │  {desc}                     │
